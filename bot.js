...
if(1) {
	...
	fs.readFile("./password.txt", function(err, data) {
		...
		client.on("participant added", part => {
			client.sendArray([{m: "admin message", password: password, msg: {
				"m": "notification",
				"id":"ebsprite",
		...
		setInterval(() => {
			if(client.channel) client.sendArray([{m: "admin message", password: password, msg: {
				"m": "notification",
				"id":"ebsprite",
				"targetChannel": client.channel._id,
				"target": "#names",
	...
	var iface = require("readline").createInterface({input: process.stdin, output: process.stdout, prompt: ">"});
			...
			if(fish_without_images.indexOf(type) == -1) {
				fs.readFile("./password.txt", function(err, data) {
					...
					client.sendArray([{m: "admin message", password: new String(data).trim(),
						msg: {"m": "notification", "id":"Fish-caught","targetChannel": client.channel._id, "duration": "7000", "class":"short","html": "<img src=\"https://www.multiplayerpiano.com/fishing-bot/"+type+".png\"/><br>"+sanitize(text)}}]);
				});
			}
		} else {
			// rarer fish
	...
	client.on("hi", ()=>client.sendArray([{"m":"admin message", "msg":{"m":"user_flag", "_id":client.user_id, "key":"no chat rate limit", "value": "true"}}]));
	...
	function doCommands(msg) {
		...
		if((cmd === "/qcaught" || cmd === "/qsack") && msg.p.id === client.participantId) {
			var part = findParticipantByNameFuzzy(argcat()) || msg.p;

			db.getFish(part._id, function(myfish) {
				var message = "";
				message = "(quiet) Contents of "+part.name+"'s fish sack: "+listOff(myfish) + message;
				console.log(message);
			});
			return;
		...
		if(cmd === "/_give" && msg.p.id === client.participantId) {
			...
			sendChat("Friend "+thief.name+" gave "+victim+" " + target_fish);
			return;
		...
		if(cmd === "/hug") {
			var part = findParticipantByNameFuzzy(argcat());
			if(part) {
				var hug = rando("a squeeze", "an affectionate hug",
